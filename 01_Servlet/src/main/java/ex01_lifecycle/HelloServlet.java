...
public class HelloServlet extends HttpServlet {
    ...
    /*
    	1. 생성자
    		1) 가장 먼저 호출된다.
    		2) 성생자 호출 뒤 자동으로 init() 메소드가 호출된다.
     */
    public HelloServlet() {
        super();
	...
	/*
	 	4. doGet()
	 		...
	 		2) GET 방식의 요청의 예시
	 			...
	 			(4) open('http://localhost:9090/01_Servlet/HelloServlet', 'target', '특징')
	 			(5) $.ajax({
	 						type: "GET",
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 클라이언트로 정보(텍스트) 보내기 위한 출력스트림 : response.getWriter() - PrintWriter
		
		// 출력스트림으로 정보(텍스트)를 보내는 메소드 : append(), write(), print(), println()
